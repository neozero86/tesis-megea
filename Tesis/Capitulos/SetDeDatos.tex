...
Para el análisis se tomó una ventana de $-4000$ $ms$ a $+4001$ $ms$ para las neuronas \glslink{VTA}{VTA} y de $-4000$ $ms$ a $+3000$ $ms$ para las neuronas \glslink{PFC}{PFC}, siendo $0 ms$ el inicio del estímulo. La duración del mismo es de $1000 ms$.
\\\\
...
{\gls{MCf} de una neurona \glslink{VTA}{VTA} con las cuatro clases, donde se observa la poca cantidad de trials GOi y NOGOi, en comparación con las otras dos clases.}
...
Utilizando la opción A, la \gls{PDC} obtenida fue de $0.5354 \pm 0.0956$.\\
Utilizando la opción B, la \gls{PDC} fue de $0.5910 \pm 0.1179$. \\\\
La \gls{PDC} total se calculó como el promedio de la \gls{PDC} individual de clasificación entre las 153 neuronas $\pm$ su desvío estándar. \\\\\\
Para el set de neuronas \glslink{PFC}{PFC}:\\
Utilizando la opción A, la \gls{PDC} obtenida fue de $0.5072 \pm 0.0485$.\\
Utilizando la opción B, la \gls{PDC} fue de $0.5433 \pm 0.0755$. \\\\
La \gls{PDC} total se calculó como el promedio de la \gls{PDC} individual de clasificación entre las 95 neuronas $\pm$ su desvío estándar. \\\\
...
La construcción se realiza para el \glslink{VTA}{área VTA} para simplificar la explicación, pero el procedimiento aplica de igual manera al \glslink{PFC}{área PFC}.
...
Se tiene la matriz $X$ con 153 columnas (cantidad total de neuronas \glslink{VTA}{VTA} que se registraron en las 30 sesiones) y 46 filas que representan 23 trials GOc y 23 trials NOGOc. También se cuenta con el vector $Y$ con 46 posiciones donde en cada posición se colocará un 1 o un 2, dependiendo si el trial (fila de $X$) correspondiente a este número de posición corresponde a la clase 1 (GOc) o a la clase 2 (NOGOc). \\
...
Se entrenó un LDA con las matrices mencionadas anteriormente y se consiguió una \gls{PDC} alrededor del 90\%. \\
