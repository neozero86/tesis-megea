...
En esta sección se describirán los aspectos fundamentales del funcionamiento de los cuatro \glspl{MDC}, todos \gls{AEAA}, con que se analizarán los datos: Discriminante Lineal de Fisher (LDA), Bayes Naive, Máquinas de Soporte Vectorial (SVM) y Random Forest.

\section{Discriminante Lineal de Fisher}
\label{capMetodos:sec:LDA}

El método del discriminante lineal utilizado es una generalización del descripto originalmente por Fisher \citep{fisher1936use}. El mismo resulta óptimo en el caso de variables con distribución normal y permite encontrar el \gls{H} que mejor separa las dos clases, siendo posible la generalización para un número mayor de clases \citep{rao1948utilization}.\\
En la figura~\ref{capMetodos:fig:LDA} se ilustra el caso de dos poblaciónes diferentes (A y B) que se pretende separar con un \gls{H}.\\
...
Uno de los problemas que surgen al utilizar esta herramienta para separar clases es la inversión de la suma de las \glspl{MC}. Cuando el número de muestras es pequeño (como en el caso de la cantidad de \textit{trials} que un animal realiza en una sesión de entrenamiento), la inversión de dicha matriz suele estar \gls{MMC}. En estos casos, se puede utilizar la \gls{MPI} de la matriz suma, dando como resultado una discriminación pseudolineal del problema \citep{liu2007efficient}. Dada la sencillez del procedimiento para encontrar el \gls{H} que mejor separa dos clases, este método es ampliamente aplicado en gran variedad de señales de origen neuronal \citep{aggarwal2013state,velliste2014motor}.


\section{Bayes Naive}
\label{capMetodos:sec:BN}

En este método se asume que las variables (\glspl{F}) son independientes entre sí. Por esta suposición que generalmente no se cumple pero que simplifica radicalmente el proceso de estimación, se considera al método ``ingenuo'' (\textit{naive}).\\
\textit{Bayes Naive} utiliza una función de distribución a priori, el teorema de Bayes y la suposición previa para poder predecir la función de distribución a posteriori de las variables \citep{duda1973pattern}. La utlización de \textit{Bayes Naive} gana importancia cuando la dimensión del espacio de \glspl{F} es alta, debido a que la estimación de la función densidad de probabilidad se vuelve poco eficiente \citep{hastie2009elements}. El método resulta rápido, incremental y apto para el trabajo con atributos discretos y continuos \citep{kononenko1993successive}, y tiene excelente \gls{PDC} cuando es aplicado a datos provenientes de variables continuas y normalmente distribuidas. 
...
Al empezar a clasificar, se utiliza la probabilidad a priori. Generalmente es determinada por la cantidad de apariciones de cada clase en el corpus de entrenamiento. El punto ``A'' es determinado por esta probabilidad. Luego se evalúan cada uno de los \glspl{F} de la muestra a clasificar. La aparición del \gls{F} ``F1'' indica una cierta probabilidad favorable a la clase 1, este \gls{F} mueve el resultado hacia el punto ``B''. La aparición del \gls{F} ``F2'' vuelve a correr la probabilidad, esta vez hacia el punto ``C''. Dado que en la muestra, los únicos dos \glspl{F} que se encontraron son ``F1'' y ``F2'', se da la clasificación por terminada. La Clase 3 es la que está mas cerca del punto ``C'', por lo tanto el resultado devuelto por el \gls{C} será que la muestra ``m'' pertenece a dicha clase.\\

Este método fue utilizado para clasificación de \textit{\glspl{S}} en forma semiautomática en \citet{harris2000accuracy} con buenos resultados, superando la clasificación manual. A su vez, en \citet{hu2005feature} se lo utilizó para predecir la decisión de una rata clasificando los \textit{\glspl{S}} de las neuronas del \gls{ACM}. En esa ocasión se lo combinó con un \gls{PCA} para extraer las mejores \glspl{F} y de esta manera competir con la \gls{PDC} obtenida con \glspl{C} mas robustos.\\ Al igual que con el \Gls{C} Lineal de Fisher, este método es ampliamente utilizado debido a su velocidad de entrenamiento y clasificación.\\ Se pueden encontrar mas ejemplos de su aplicación en procesamiento de señales neuronales en \citet{schmuker2014neuromorphic, valenti2006automatic}

\section{SVM}
\label{capMetodos:sec:SVM}

El método \textit{Support Vector Machine} (SVM) se basa en la idea de proyectar los datos en un espacio dimensional mayor, con el fin de encontrar un \gls{H} que permita separar las clases. Las características del \gls{H} aseguran una alta capacidad de generalización de este \glslink{MAA}{método de aprendizaje automático} \citep{cortes1995support}.
\\
...
Se busca que el margen sea ancho para una mayor capacidad de generalización. Para lograrlo, SVM utiliza una condición de márgenes relajados \textit{"soft margin"} que permite que haya algunos elementos de las clases a separar dentro de los márgenes (puntos a1, a2 y b1 en la figura~\ref{capMetodos:fig:SVM}). El balance entre maximizar el margen y minimizar la cantidad de elementos dentro del mismo impacta en la \gls{PDC} y en el error de clasificación.\\
...
\end{enumerate}
\vspace{2 mm}
Debido a que este es un método de clasificación mas robusto que los métodos enunciados anteriormente \citep{hu2005feature}, es ampliamente utilizado para el procesamiento de señales neuronales. En \citet{vogelstein2004spike} se lo utiliza en el proceso de ordenamiento de \textit{\glspl{S}} y reducción de ruido, con resultados superiores a las técnicas de ordenamiento de \textit{\glspl{S}} basadas en ``template-matching''. Otro ejemplo es el de resolución de superposiciones en ordenamiento de \textit{\glspl{S}} descripto en \citet{ding2008spike}.\\
También es utilizado en la detección de \textit{\glspl{S}} en señales electroencefalográficas \citep{acir2004automatic}.\\\\
\noindent
...
El método de \textit{Random Forest} utiliza la \gls{DQ} (\textit{divide and conquer}). Se trata de un ensamble de \gls{AD} entrenados con valores provenientes de un vector de muestras y variables aleatorias, tomadas de forma independiente y con la misma distribución para todos los árboles \citep{breiman2001random}. De esta manera, mediante un sistema de votos, se logra asignar la clase correspondiente a cada una de las muestras. Como los \gls{AD} son predictores sin sesgo pero tienen alta \mbox{varianza}, Random Forest busca reducir la varianza promediando un alto número de \gls{AD} \citep{hastie2009elements}. De esta forma, el \gls{EG} tiende a un límite cuanto mayor sea el número de árboles \citep{breiman2001random}.
...
Cada variable (\gls{F}) ``f'' de la muestra a clasificar es evaluada en cada árbol, recorriendo en forma binaria las ramas, hasta llegar a una hoja. Estas hojas determinan la probabilidad que la muestra pertenezca a la clase ``c'', dado que esta posee el \gls{F} ``f''. Esta probabilidad constituye el voto de cada árbol para la clase c. Recorriendo el vector de \glspl{F} y sometiendo cada \gls{F} de la muestra al mismo proceso de votación, se obtiene, mediante la suma de todos los votos, la decisión acerca de la clase a la que pertenece la muestra. \citep{kim2013fire,breiman2001random}. \\

Este método se ha utilizado tanto para la clasificación, como para la selección de las \glspl{F} mas relevantes en el procesamiento de señales neuronales. En \citet{oh2003estimating} fue empleado para reducir el número de variables en un ensamble de tren de \textit{\glspl{S}}. A su vez en \citet{lehmann2007application, fraiwan2012automated} se utilizó Random Forest en señales electroencefalográficas, para detectar la enfermedad del Alzheimer.