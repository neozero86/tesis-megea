...
En la Tabla~\ref{capResultados:fig:mediaComp} se muestra el valor \textit{p} con el cual se puede evidenciar que un método $M_i$ funciona mejor que otro $M_j$. Esta significancia se calculó contando las veces en que la media de la performance de una ventana para uno de los métodos, fue más alta que la del otro, es decir, sin asumir ninguna distribución paramétrica:
...
%-------------------------------------------------------------------
\subsection{Corteza PreFrontal}
%-------------------------------------------------------------------
\label{capResultados:sec:analisisVariabilidad:subsec:PFC}

El análisis precedente se repitió sobre una población de neuronas registradas simultáneamente en la corteza PreFrontal. En la figura~\ref{capResultados:fig:AllMethodsPFC} se puede ver el desempeño de los cuatro métodos. Se grafica la performance promedio de las 500 corridas por cada ventana y el B.S.E.M.
